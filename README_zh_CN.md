...
mergerfs - 一个功能丰富的联合文件系统


# SYNOPSIS

mergerfs -o&lt;选项&gt; &lt;文件夹列表&gt; &lt;挂载点&gt;


# DESCRIPTION

**mergerfs** 是一种联合文件系统，用来对“文件分散在各类存储上”的问题进行简化其存储和管理。跟 **mhddfs**、**overlayfs**、**unionfs**、**aufs** 有些类似。


# 特性（FEATURES）

* 运行在用户空间 (FUSE)
* 行为可配置 / file placement
* 支持扩展属性 (xattrs)
* 支持文件属性 (chattr)
* 可实时配置 (通过 xattrs)
* 用 root 运行也很安全
...
* 分支 branch: A base path used in the pool.
* 池 pool: The mergerfs mount. The union of the branches.
* 相对路径 relative path: The path in the pool relative to the branch and mount.
* 策略 policy: The algorithm used to select a file when performing a function.
* 函数 function: 对文件系统的一个调用 (open, unlink, create, getattr, etc.)
* 类别 category: A collection of functions (action, create, search).
* 路径保留 path preservation: Aspect of some policies which includes checking the path for which a file would be created.
...
#### 当你需要 `mmap` 时 (used by rtorrent and some other programs)
...
* **config**: 配置文件的路径. 配置文件中的配置项格式为 `选项=值` 格式，下面的选项也一样.
* **branches**: 用冒号隔开的文件夹路径列表.
...
* **fsname=STR**: Sets the name of the filesystem as seen in **mount**, **df**, etc. Defaults to a list of the source paths concatenated together with the longest common prefix removed. （译者注：STR 可以使用汉字）
...
* BOOL布尔 = 'true' | 'false'
* INT整数 = [0,MAX_INT]
* SIZE大小 = 'NNM'; NN = INT, M = 'K' | 'M' | 'G' | 'T'
* STR字符串 = string
* FUNC函数 = FUSE function
* CATEGORY类别 = FUSE function category
* POLICY策略 = mergerfs function policy


### 分支（branches）

'branches' (以前叫 'srcmounts') 参数是一个以冒号 (':') 隔开的路径列表。路径是否在同一个磁盘上并没有关系，是否是相同的文件系统也没有关系。Used and available space will not be duplicated for paths on the same device and any features which aren't supported by the underlying filesystem (such as file attributes or extended attributes) will return the appropriate errors.
...
上面的命令将使用 /mnt 文件夹下的 **cdrom** 以及所有以 **disk** 开头的路径。
...
# 函数FUNCTIONS / 策略POLICIES / 类别CATEGORIES
...
策略，如下所述，分为两种类型：路径预留（`path preserving`）和 非路径预留（`non-path preserving`）。

所有名称以 `ep` 开头的策略 (**epff**, **eplfs**, **eplus**, **epmfs**, **eprand**) are `path preserving`. `ep` stands for `existing path`.
...
* https://github.com/trapexit/mergerfs-tools
  * mergerfs.ctl: 一个用来在运行时查询和配置 mergerfs 的工具
